...
import { getAuthenticatedHeaders, getShopDomain } from '../utils/shopifyAuth';

// Helper to construct headers (updated to use new auth system)
const getHeaders = async (accessToken?: string) => {
  return await getAuthenticatedHeaders(accessToken);
};
...
export const getShopifyProducts = async (): Promise<ShopifyProduct[]> => {
  ...
  const shopDomain = settings.shopDomain || getShopDomain();

  if (!shopDomain) {
    console.warn("[Shopify API] Shop domain missing. Please configure in Settings.");
    return [];
  }

  try {
    const headers = await getHeaders(settings.accessToken);
    console.log('[Shopify API] Request headers prepared');

    const response = await fetch(`${getBaseUrl(shopDomain)}/products.json?limit=20`, {
      method: 'GET',
      headers
    });
    ...
    console.log('[Shopify API] ✓ Successfully fetched', data.products?.length || 0, 'products');
    return data.products as ShopifyProduct[];

  } catch (error) {
    console.error("[Shopify API] Failed to fetch products:", error);
    return [];
  }
};
...
export const getShopifyOrders = async (): Promise<Order[]> => {
  ...
  try {
    ...
    const response = await fetch(`${getBaseUrl(shopDomain)}/orders.json?status=any&limit=20`, {
      method: 'GET',
      headers
    });
...
export const updateProductHSCode = async (productId: string, hsCode: string): Promise<boolean> => {
  ...
  try {
    ...
    const url = `${getBaseUrl(shopDomain)}/products/${productId}/metafields.json`;
    ...
    console.log('[Shopify API] Update response status:', response.status);

    if (!response.ok) {
      const errorText = await response.text();
      console.error('[Shopify API] Update error:', errorText);
      throw new Error(`Failed to save metafield: ${response.statusText}`);
    }

    console.log('[Shopify API] ✓ Successfully updated HS Code');
    return true;

  } catch (error) {
    console.error("[Shopify API] Failed to update product:", error);
    return false;
  }
};
